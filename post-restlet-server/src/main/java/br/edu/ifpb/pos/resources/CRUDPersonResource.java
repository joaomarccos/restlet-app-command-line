 ...
 */
public class CRUDPersonResource extends ServerResource {
    ...
    public StringRepresentation insert(Representation r) {
        try {
            ...
            Logger.getLogger(CRUDPersonResource.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public StringRepresentation update(Representation r) {
        try {
            String data = r.getText();
            DaoFactory.createPersonDao().atualizar(JsonConverter.convertToPerson(data));
        } catch (Exception ex) {
            Logger.getLogger(CRUDPersonResource.class.getName()).log(Level.SEVERE, null, ex);
            return new StringRepresentation(Results.ERROR.json(), MediaType.APPLICATION_ALL_JSON);
        }
